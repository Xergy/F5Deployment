...
if ($licenseType -eq "BYOL") {
  ...
  $deployment = New-AzureRmResourceGroupDeployment -Name $resourceGroupName `
    ...
    -imageName $imageName `
    -bigIpVersion $bigIpVersion `
    -numberOfAdditionalNics $numberOfAdditionalNics `
    -additionalNicLocation $additionalNicLocation `
    -numberOfExternalIps $numberOfExternalIps `
    -vnetName $vnetName `
    -vnetResourceGroupName $vnetResourceGroupName `
    -mgmtSubnetName $mgmtSubnetName `
    -mgmtIpAddressRangeStart $mgmtIpAddressRangeStart `
    -externalSubnetName $externalSubnetName `
    -externalIpSelfAddressRangeStart $externalIpSelfAddressRangeStart `
    -externalIpAddressRangeStart $externalIpAddressRangeStart `
    -internalSubnetName $internalSubnetName `
    -internalIpAddressRangeStart $internalIpAddressRangeStart `
    -tenantId $tenantId `
    -clientId $clientId `
    -servicePrincipalSecret $servicePrincipalSecretSecure `
  ...
  $deployment = New-AzureRmResourceGroupDeployment `
    ...
    -tagValues $tagValues `
    -allowUsageAnalytics $allowUsageAnalytics `
    -licensedBandwidth "$licensedBandwidth"
